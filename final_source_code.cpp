...
void multiplication(string instruction, int a[]);
...
int main() {
    ...
    for (i = 0; i < memory.size(); i++) {
        ...
        else if (memory[i].instruction.find("mul") != -1 || memory[i].instruction.find("mulh") != -1 ||
            memory[i].instruction.find("mulhu") != -1 || memory[i].instruction.find("mulhsu") != -1) {

            multiplication(memory[i].instruction, regArray);
        }
        else if (memory[i].instruction.find("ecall") || memory[i].instruction.find("ebreak") ||
            memory[i].instruction.find("fence")) {
            cout << endl << "Encountered Halting Instruction: end program." << endl;
            return 0;
        }
...
int GetLSB(int intValue)
{
    return (intValue & 0x00000000FFFFFFFF);
}
int GetMSB(int intValue)
{
    return (intValue & 0xFFFFFFFF00000000);
}
void multiplication(string instruction, int a[]) {
    ...
    int rd = translate(result[1]);
    int rs1 = translate(result[2]);
    int rs2 = translate(result[3]);
    ...
    else if (result[0] == "mulh") {
        product = a[rs1] * a[rs2];
       long long int temp = GetMSB(product);
       for (int i = 0; i < 4; i++) temp = temp >> 8;
       a[rd] = temp;
    } 
    else if (result[0] == "mulhu") {
        product = (unsigned) a[rs1] * (unsigned) a[rs2];
        long long int temp = GetMSB(product);
        for (int i = 0; i < 4; i++) temp = temp >> 8;
        a[rd] = temp;
    }
    else if (result[0] == "mulhsu") {
        product = a[rs1] * (unsigned)a[rs2];
        long long int temp = GetMSB(product);
        for (int i = 0; i < 4; i++) temp = temp >> 8;
        a[rd] = temp;
    }
